...
import type { PlumeThemeHomeBanner,PlumeThemeHero } from 'vuepress-theme-plume'
import { computed } from 'vue'
import { withBase } from 'vuepress/client'
import { isLinkHttp } from 'vuepress/shared'
import { useData } from 'vuepress-theme-plume/composables'

interface ExtendedPlumeThemeHero extends PlumeThemeHero {
    intro?: string; 
}
...
const intro = computed(() => (props.hero as ExtendedPlumeThemeHero)?.intro ?? (matter.value.hero as ExtendedPlumeThemeHero)?.intro);
</script>
...
.vp-home-banner .content .hero-intro {
  width: 100%;
  max-width: 700px;
  margin-top: 0.5rem;
  font-size: 16px;
